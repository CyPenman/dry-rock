...
describe('rankCragDays', () => {
  ...
  it('never ranks a low-confidence day above a high-confidence one, even with a lower score', () => {
    const entries = [
      { crag: portland, day: makeDay({ score: 0.95, confidence: { agreeCount: 1, total: 4, fraction: 0.25 } }) },
      { crag: stanage, day: makeDay({ score: 0.4, confidence: { agreeCount: 4, total: 4, fraction: 1 } }) },
    ];
    const ranked = rankCragDays(entries, null);
    expect(ranked[0].crag.id).toBe('stanage');
  });

  it('sorts by score within the same confidence tier', () => {
    const entries = [
      { crag: portland, day: makeDay({ score: 0.4 }) },
      { crag: stanage, day: makeDay({ score: 0.8 }) },
    ];
    const ranked = rankCragDays(entries, null);
    expect(ranked[0].crag.id).toBe('stanage');
  });
  ...
  it('drops entries with no day result and attaches distance when home is set', () => {
    const entries = [
      { crag: portland, day: makeDay() },
      { crag: stanage, day: null },
    ];
    const ranked = rankCragDays(entries, { lat: 53.38, lon: -1.47 }); // Sheffield-ish
...
describe('sortByWorthTheDrive', () => {
  it('can promote a lower-scoring but much closer crag over a higher-scoring distant one', () => {
    ...
    const stanage = CRAGS.find((c) => c.id === 'stanage')!;
    const ranked = [
      { crag: portland, day: makeDay({ score: 0.9 }), distanceKm: 400 },
      { crag: stanage, day: makeDay({ score: 0.8 }), distanceKm: 5 },
    ];
